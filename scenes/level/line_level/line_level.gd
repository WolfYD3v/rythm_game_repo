...
func _ready() -> void:
	try_load_level_data()
	lines_level_health_bar.init_health_bar(
		all_obstacle_sets.obstacle_sets.size()
	)
	...
	if not instant_gameplay:
		...
		await get_tree().create_timer(3.5).timeout
	else: await get_tree().create_timer(3.5).timeout
	
	# Play the music, and start the recursive obstacles generation
	audio_stream_player.play()
	lines_level_obstacle_generation.start_generation(all_obstacle_sets.obstacle_sets)
...
func try_create_obstacle(obstacle_data: LineLevelObstacle) -> void:
	...
	new_obstacle.position.x = 3.0 * (obstacle_data.line - 1)
